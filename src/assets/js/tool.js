...
const isMJ = () => {
  ...
  if (sessionStorage.getItem('browseType')) {
    sta = true;
  } else if (('android' in window && 'clickOnAndroid' in window.android)) {
    sta = true;
  } else if (('webkit' in window && 'messageHandlers' in window.webkit && 'Native' in window.webkit.messageHandlers && 'postMessage' in window.webkit.messageHandlers.Native))
  return sta;
};
...
const workIng = url => {
  if (isMJ() && url) {
    callApp({
      action: 'general_links',
      data: {url: url}
    })
  } else {
    Vue.$vux.toast.text('该功能正在开发中', 'bottom');
  }
};

/**
 * lw 需要用到用户登录验证的可以调用此方法
 * @param auto 是否自动跳转到登录,默认假
 * @returns {boolean}
 */
const loginState = (auto = false) => {
  if (!Global.getSessionId() || Global.getSessionId().length < 1) {
    if (auto) {
      if (isMJ()) {
        callApp({
          action: 'login',
        ...
        // let result = `${shareUrl}${router.history.current.fullPath}`;
        // window.location.href = `/login/login?isSession=1&jumpType=1&result=${encodeURIComponent(result)}`;
        window.location.href = `/login/login?isSession=1&jumpType=1&result=${escape(window.location.href)}`;
      }
    } else {
      return false;
    }
